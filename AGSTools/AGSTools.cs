...
namespace AGS
{
    public class AGSTools
    {
            ...
            using (FileStream fs = new FileStream(filename, FileMode.Open))
            {
                ...
                using (BinaryReader br = new BinaryReader(fs))
                {
                    ...
                    foreach (int scomyPos in SCOMY_Positions)
                    {
                        ...
                        //newTestData = Replace(newTestData, new byte[] { 0x03 }, new byte[] { 0x0D, 0x0A });
                        //newTestData = Replace(newTestData, new byte[] { 0x1D }, new byte[] { 0x0D, 0x0A });
        ...
        private static void ConvertCharToByte(char[] chars, byte[] bytes)
        {
            int x = 0;
        ...
        public static void CreateTRA_File(Gameinfo info, string filename, Dictionary<string, string> entryList)
        {
            using (FileStream fs = new FileStream(filename, FileMode.Create))
            {
                ...
                char[] cGameTitle2 = GameTitle.ToCharArray();
                //GameTitle.CopyTo(0, cGameTitle, 0, GameTitle.Length);
                EncryptText(cGameTitle2);
                ...
                ConvertCharToByte(cGameTitle2, bGameTitle);
        ...
        private static void DecryptText(char[] toEnc)
        {
            int adx = 0;
            int toencx = 0;
